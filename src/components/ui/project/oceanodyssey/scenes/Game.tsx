...
function InventoryBackground({ screenWidth, screenHeight }: InventoryBackgroundProps) {
	const drawShape = (graphics: Graphics) => {
		if (graphics) {
			graphics.clear();
			graphics.beginFill(0x0A0AFF);
			graphics.drawRect(0, 0, screenWidth * 0.15, screenHeight);
			graphics.endFill();
		}
	};
...
function Game({
	...
	// Step 2: Move the shape based on WASD key press
	useEffect(() => {
	...
	useTick(() => {
		...
		setPositionText(`Pos - X: ${playerPosition.x}, Y: ${playerPosition.y}`);
	});
